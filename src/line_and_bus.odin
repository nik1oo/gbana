...
Bus:: struct($T: typeid) {
	...
	latency: int,
	callback: proc(new_output: T) }
Bus_Data:: struct($T: typeid) {
	data:    T,
	latency: int }
bus_init:: proc(bus: ^Bus($T), latency: int = 1, callback: proc(new_output: T) = nil) {
	bus.output = {}
	bus.latency = latency }
...
bus_tick:: proc(bus: ^Bus($T)) {
	for queue.len(bus._queue) > 0 {
		...
		if bus_data.latency == 0 {
			...
			if (bus.output != old_output) && (bus.callback != nil) do bus.callback(bus.output)
